...
在 Windows 10 / macOS / Ubuntu 三種平台上，自動辨識平台並執行對應邏輯，透過 adb 連接 Android 裝置，掃描裝置檔案系統中「指定資料夾路徑」下的所有 `.apk` 檔案，針對每一台裝置輸出一份獨立的 txt 純文字檔（檔名格式：`{型號}_{序號}_{YYYYMMdd}-{HHmmss}.txt`）。檔案開頭記錄裝置資訊（Model / Serial / OS Version / API Level / APK Count），接著每個 apk 一個區塊，包含 Display Name、Package Name、Version Name、Version Code、File Path。
...
2. **掃描範圍**：掃描裝置檔案系統中「指定資料夾路徑」下的實體 `.apk` 檔案（非僅已安裝套件清單）。預設路徑為 `/system/priv-app`，可用 `--paths` 參數覆寫（逗號分隔多個路徑）。
